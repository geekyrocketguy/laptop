...
t_start = datetime.datetime(2025, 11, 10, 7).timestamp() #Time of permit release. year, month, day, hour, [min, sec] converted to seconds in epoch. 
t_in_sec = [-60, -5, -2, 10, 60, 120, 60*60] #seconds, define refresh rates and times
t_refresh = [10, 1,  0,  1,  10, 60] #seconds, define refresh rates and times

month = 4 #what month of campsites to scrape?
year = 2026 #what year of campsites to scrape?


#t_start = datetime.datetime(2025, 11, 9, 17, 30).timestamp() #Time of permit release. year, month, day, hour, [min, sec] converted to seconds in epoch. 
#t_in_sec = [-60, -5, -2, 10, 60, 120, 60*60] #seconds, define refresh rates and times
#t_refresh = [10, 1,  0,  1,  10, 60] #seconds, define refresh rates and times

#month = 11 #what month of campsites to scrape?
#year = 2025 #what year of campsites to scrape?

url_base = "https://www.recreation.gov/api/permititinerary/4675315/division/4675315"
campsite_nums = (np.arange(20)+30).astype(str) #31-50
    ...
    for i in range(len(t_in_sec)-1):
        ...
        while time.time() < t_start + t_in_sec[i+1]:
            ...
            for j in range(len(campsite_nums)):
                url = url_base + campsite_nums[j] + "/availability/month?month=" + str(month) + "&year=" + str(year)
                ...
                try:
                    ...
                    filename = data_dir + '/' + campsite_names[j] + ' ' + now_str + ".json"
