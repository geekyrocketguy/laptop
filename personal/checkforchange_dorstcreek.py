...
url='https://www.recreation.gov/api/camps/availability/campground/232460/month?start_date=2022-07-01T00%3A00%3A00.000Z'
...
campground_names = ['Dorst Creek Campground']
...
if not os.path.isfile('status_dorstcreek.txt'): #if someone deleted the file, recreate it
    f=open('status_dorstcreek.txt', 'w')
    f.write(message)
    f.close()
    print( 'status_dorstcreek.txt was deleted by some goon, but it has been restored.')

f=open('status_dorstcreek.txt', 'r')
...
if (message != oldcontents) or ('test' in args): #has something changed? Then email user.
    ...
    if 'test' in args:
        ...
        mysubject = 'Dorst Creek campground code is working'
    else:
        mysubject = 'Change in Campsite Availability'

    message = "The campsite availability has changed. " + message + \
           "The reservation URL is https://www.recreation.gov/camping/campgrounds/232460. We want to check in 7/2 and check out 7/3.\n\n" + \
