...
url='https://www.recreation.gov/api/camps/availability/campground/249982/month?start_date=2022-07-01T00%3A00%3A00.000Z'
...
campground_names = ['Buckeye Flat Campground']
...
if not os.path.isfile('status_buckeyeflat.txt'): #if someone deleted the file, recreate it
    f=open('status_buckeyeflat.txt', 'w')
    f.write(message)
    f.close()
    print( 'status_buckeyeflat.txt was deleted by some goon, but it has been restored.')

f=open('status_buckeyeflat.txt', 'r')
...
if (message != oldcontents) or ('test' in args): #has something changed? Then email user.
    ...
    if 'test' in args:
        ...
        subject = 'Buckeye Flat campground code is working'
    else:
        subject = 'Change in Campsite Availability'
        message = "The campsite availability has changed. " + message + \
'''The reservation URL is https://www.recreation.gov/camping/campgrounds/249982. We want to check in 7/2 and check out 7/3.
